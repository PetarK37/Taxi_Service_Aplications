...
public class PhoneReservation extends Ride {
    ...
    public PhoneReservation(GregorianCalendar dateAndTime, String startAddress, String arrivalAddress, double numberOfKm, double duration, int price, ReservationStatus status, int rideNumber, Driver driver, User user,int rideRating){
        super(dateAndTime,startAddress,arrivalAddress,numberOfKm,duration,price,status,rideNumber,driver,user,rideRating);
    }

    public void printToConsole(){
        System.out.println("Datum i vreme pocetka: " + DatesController.dateAndTimeToStr(this.dateAndTime) + "\nPocetna adresa: " + this.startAddress + "\nAdresa odredista: " + this.arrivalAddress + "\nDuzina voznje: " + (this.numberOfKm == 0 ? "-" : this.numberOfKm) + "km" +  "\nTrajanje voznje: " + this.duration + "\nCena: " + (this.price ==  0 ? "-" : this.price) + "\nStatus: " + this.status + "\nBroj voznje: " + this.rideNumber + "\nVozac: " + (this.driver == null ? "-" :  this.driver.username) + "\nKorisnik: " + this.user.username +  "\n");
    }


    @Override
    public String Serialize() {
        return this.isDeleted + "," + this.rideNumber + "," + DatesController.dateAndTimeToStr(this.dateAndTime) + "," + this.startAddress + "," + this.arrivalAddress + "," + this.numberOfKm + "," + this.duration + "," + this.price + "," + this.status.ordinal() + "," + (this.driver == null ? "-" : this.driver.username) + "," + this.user.username + "," + this.rideRating +"\n";
    }
    ...
    public Ride Deserialize(String loadedLine) {
        ...
        if (entities.length != 12) {
            throw new FileErrorExeption("Unable to  deserilize Drive at line: " + loadedLine);
        }
